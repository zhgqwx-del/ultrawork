...
> | @agent/orchestrator | ✅ 阶段3 全量已实现（2026-06-12，含 017 Team 页） | 编排层（ADR-031）：spawn/await/steer/cancel 原语 + 治理护栏（maxConcurrent/maxDepth=1/超时/子会话 tools deny）+ DAG 调度（Pipeline/Fan-out 同一执行器，按步 worktree 隔离）+ agent 驱动 delegate（`delegate-mcp` stdio shim → 阻塞 `POST /orchestration/delegate` 回 D-2 契约；ACP per-session / opencode 全局双注入）+ QueueOwner 实现；宿主在 ACP sidecar :4099（`/orchestration/*` HTTP+SSE），Desktop `/orchestration` 两 tab（017：Team 协作 = Leader 会话委派默认化〔opencode 每轮 system+task deny / ACP `_meta.systemPrompt`〕/ 流水线 recipe）+ 主对话 delegate 卡片/dock（不侵入主聊天）；普通会话物理隔离（非 Leader opencode prompt 恒 deny `orchestrator_*`），「编排模式」开关已移除 |
