...
- **mac 真机 = `scripts/verify-close-to-background-macos.sh`（36 条断言（含「X 后让出前台」），空闲桌面上四轮 + 三轮全绿；AX 读状态 + CGEvent 真实输入〔`scripts/macos-hid.swift`〕驱动原生窗口，断言取自 `pgrep` / `lsof` / AX 窗口数 / `com.apple.spaces`，含「启动失败 ⇒ X 真退出」反向臂 + 全屏下快速连按 Cmd+W）**：
  1. 点 X → app pid 与 4 个 sidecar pid/端口一个没变，AX 窗口数 1→0，日志零 `[shutdown]`，**前台切到下一个 app** ✅
