...
新增包 `packages/core/connector`，定义后端无关控制面，下挂可插拔 backend adapter。**包装而非取代** api-client。两个 backend：

- **OpenCodeBackend** = 包装现有 `ApiClient`（REST）+ `SSEClient`（事件），REST 路径不变。
- **ACPBackend** = 包装 **重写后** 的 acp-client 前端 router（**B2 决策：不复用 `feat/acp-support` 分支的 router，在当前 main 上参考重建**）+ EventSource。
