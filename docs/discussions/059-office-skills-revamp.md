...
| **S2** 🟡 | `pdf` 技能重做 —— **第一刀**：P1/P2/P4（2026-08-01，`071cd18c`）；**第二刀**：表单族 P5-P10（2026-08-01）。剩 5 项 pending。见 §六·补 | pdf/ 骨架 + 表单族 + L0/L1/L2 全绿 | 进行中 |
...
1. **P13 + P14 一起做**（字体嵌入不解决，生成能力产出的就是「只在本机不豆腐」的 PDF），
   顺带把 P1/P4 的产物变成 L2 能读的东西。**前置：CJK 字体的许可与体积路线要先拍板。**
2. **P3 表格抽取**（与 xlsx 的表模型一起定，S3）。
