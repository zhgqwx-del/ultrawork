...
**为什么撤而不是继续查**：main 不该停在红色，而这条链路只影响横幅文案的精细度 —— 撤下后横幅仍然出现，只是对「sidecar 死了」也显示通用的「已断开」+「重新连接」。这是已知的**误导性 UI**（点重连不会有用），记在遗留里而不是假装无事。
...
- **「sidecar 已退出」与「网络断开」未区分**（上一节）。可考虑的替代实现：不靠 Rust 事件，改由 renderer 在断流时探一次 `/global/health` —— 连接被拒 ⇒ 进程没了，有响应 ⇒ 进程活着只是流断了。全在前端、无 Windows 链接风险，且反映的是当下事实而非一次性事件。
