...
3. **成员约束强度** → ✅ **已拍板（2026-06-12）：MVP 仅提示约束**（system 提示成员清单 + 治理护栏兜底；服务端白名单留观察后再加）。
4. **普通会话 deny 的范围** → ✅ **已拍板（2026-06-12）：所有非 Leader 的 opencode prompt 一律带 `orchestrator_*` deny**——desktop 普通会话 + gateway/IM 渠道会话统一（IM 暂无编排诉求，一并 deny 防意外）；ACP 普通会话本就不注入无需处理。
5. **「编排模式」开关** → ✅ **已拍板（2026-06-12）：直接移除**（全局 MCP 注册降级为 Team 功能内部静默 ensure）。

> **§5 五项全部拍板完毕（2026-06-12）——本提案可作为阶段3 第三批 scope 开工**；落地时回写 ADR-031（D-3 opt-in 的最终产品形态 = Team 页）。
