...
### Fixed
- **Home 工具行布局两处（018 走查发现，2026-06-13）**：① 「马上开始」CTA 被工具行 chip 挤压成竖排单字——CTA 加 `shrink-0 whitespace-nowrap`、chip 区改 `min-w-0 flex-1 overflow-x-auto`（窄窗口放不下时 chip 横向滚动，CTA 永不压缩）；② AgentSelector 下拉行高不齐（无描述的 OpenCode 单行矮、有描述的其余双行高）——行加 `min-h-[2.75rem]` 居中统一（实测 44px 对齐，差 0px）。窄窗口 900px GUI 验证 2/2
