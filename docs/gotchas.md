...
- **Sidecar 进程生命周期 / 退出清理（2026-06-17 真机实测，`lib.rs`）**：
  ...
  - **关窗口 ≠ 退出 app（2026-09-15 起，discussions/061 / ADR-074，`background.rs`）**：X / Cmd+W / Alt+F4 走 `CloseRequested → prevent_close + hide`，进程与 4 个 sidecar 常驻；真正退出 = Cmd+Q / Dock 右键 / 托盘「退出」/ 系统关机 / SIGTERM。**四条读源码才拿得到的契约**：① Cmd+Q 是 muda `terminate:` → tao `applicationWillTerminate` → `LoopDestroyed` → `RunEvent::Exit`，**不经过 `CloseRequested`/`ExitRequested`**，拦 X 拦不到它；Windows 关机是 `WM_ENDSESSION → loop_destroyed()`，同理。② Dock 点击 = tao `Reopen{has_visible_windows}` 且 tao **向 AppKit 返回该值**——隐藏/最小化时返回 NO ⇒ AppKit 什么都不做，必须自己 `unminimize+show+set_focus`。③ `window.hide()` **不碰 WebView2 的 `IsVisible`**（`tauri-runtime-wry` 只发 `tao.set_visible(false)`；wry 只在 `Webview::hide` 时才 `SetIsVisible(false)`）⇒ Windows 必须连 webview 一起 hide，否则隐藏后 Chromium 照常离屏合成。④ Linux 托盘库 `libappindicator-sys` 缺库是 **`panic!` 不是 Err**，且 `Lazy` poison 后再碰仍 panic ⇒ 只在 `setup()` 试一次、`catch_unwind` 包住、结果进 `TRAY_READY`；没托盘的非 mac 平台关 = 退（用户不能失去唯一可见入口）。⑤ **macOS 原生全屏没有外部可读的「退出完成」信号**：tao `set_fullscreen(false)` **请求时**就把 `is_fullscreen()` 翻成 false（`tao/window.rs:1257` 先写目标态再异步 toggle），AppKit 的 `styleMask` 全屏位在退出**开始** ~15ms 就清，而 `orderOut:` 在 ~1s 动画中要么被丢弃要么（近尾声）被接受但窗口仍标全屏 ⇒ 唤回后还是全屏；快速连按还会让 tao 的 transition 记账失步、下一次 `set_fullscreen` 被吞。所以**全屏下关窗 = 只退全屏**（判据用 `NSWindow.styleMask`、动作直接 `toggleFullScreen:`，都别经 tao），不要在动画期间 hide，也别用定时器猜。**AX `AXPress` 绿色按钮 / AX `keystroke` 会把 AppKit 全屏状态机搞进半状态**（之后任何来源的 toggle 都被吞、只剩 ⌃⌘F 能救），验全屏行为必须用 CGEvent 真实输入（`scripts/macos-hid.swift`）。⑦ **macOS 只 `orderOut` 最后一个窗口时 app 仍是活动应用**：菜单栏挂着 app 名却没窗口，且 `NSApp.requestUserAttention` 对活动应用**不跳 Dock** ⇒ 隐藏后必须 `app.hide()`（Cmd+H 语义）让出激活，唤回先 `app.show()`。⑥ **Windows 合作式 `WM_CLOSE`（taskkill 无 /F、任务管理器、安装器）与 X 无法区分**，都进 `CloseRequested` ⇒ 隐藏；随后的强杀不经 `RunEvent::Exit`，靠下次启动孤儿自愈。**确定的限制**：通知点击**不会**唤回窗口——tao 的 app delegate 没挂 `applicationDidBecomeActive`，`tauri-plugin-notification` 桌面端发完即忘。启动失败态（`BOOT_STAGE_FAILED`）关 = 退。**`tauri dev` 关窗不再结束进程，Ctrl+C 退。** 退出时（任一路径）Cmd+Q / 可捕获信号（SIGINT/TERM/HUP）都会清理干净 → `RunEvent::Exit` 或信号处理器调 `shutdown_sidecars()`（按 pid kill 4 sidecar + 端口兜底 + `kill_browser_mcp_processes`；MCP 子进程随 opencode 死、watchdog 自杀）→ **零残留**。
...
一份只有 1 个汉字 + 8 个这种空格的英文文档，直接算会得 1/9 ⇒ 误拒。两道防线：① **按字体**——空对象的字体若在本页画出过文本层里的 CJK 字
（`FPDFText_GetFontInfo` ↔ `FPDFFont_GetBaseFontName`，pdfium 两边都去掉子集前缀），它就是空格不是缺字（回退字体从不在文本层里有 CJK 字，所以它的空对象全算）；
拉丁字体里的行尾空格这一招认不出来，于是 ② 拿**源文档的 CJK 数**做上界：只相信 `源 CJK 数 − 文本层抽到的 CJK 数` 那么多个空对象
（正常页差值为 0 ⇒ 全被忽略；豆腐块页文本层为 0 ⇒ 全算）。**上界因此绝不能多算**：docx 只数被引用且开关打开的页眉页脚
（`first` 要 `<w:titlePg/>`、`even` 要 `<w:evenAndOddHeaders/>`，Word 常把关掉的 part 留在包里）、跳过 `mc:Fallback`（文本框存两份只画一份）、
加上脚注尾注；xlsx 按 Calc 打印的范围数（打印区域、隐藏行列、页眉页脚、数字格式里的 `"元"`），这需要**非 read_only** 加载（read_only 没有 print_area / hidden / header）。
渲染预算只花在**文本层里有 CJK 字**的页上，空对象在每一页都数（不渲染）——否则三页带项目符号的英文页会把预算吃光，第 4 页的中文根本没被量。
