...
⚠️ **`--out` 写出的是完整数据，不是给你 `cat` 的。** 超过 20 条时 stdout 只留计数和一个
指针，而那个指针会带上目标文件的**字节数** —— 先看那个数再决定怎么读它。45 格的表是
5 KB，2000×30 的表是 **6.4 MB**，`cat` 一下就把整个上下文烧掉了。要小一点的结果，
用更窄的 `--range` / `--cells` 重跑；要挑字段，用 `python3 -c` 读那个 JSON。
...
- **`xl/calcChain.xml` 在公式变化时删掉，值变化时保留。** 它是求值顺序的缓存：
  ...
  删掉等于白白让人重算一遍。
- **改一个格，所有算它的公式的缓存值会被清掉**（报告里 `caches_invalidated`）。
  这不是洁癖：把 `B2` 从 1284.6 改成 1350，`毛利` 那格里存着的还是 488.2，
  而**任何读「值」的消费者都会把它当成当前值** —— 本技能自己的 reader 标
  `uncalculated: false`，LibreOffice 导出 PDF 时**照着 488.2 画到纸上**
  （实测：它不认 `fullCalcOnLoad`）。清掉之后同一次渲染出来是 553.6，因为
  **没有缓存值的公式格会在打开时被算出来**。
  ⇒ 改完之后那些格在文件里是空的，**要真数字就跑 `xlsx_recalc.py`**。
  依赖遍历有上界（`MAX_DEPENDENCY_SPAN`）：图大到展不开时改为**清掉全部缓存公式**，
  并在报告里 `caches_invalidated_coarse` 说明 —— 宁可多清（丢掉还成立的数），
  不留一个已经不成立的数。
...
**数字也要量，而且要按它的数字格式量**：`1.0877…` 在 `0.0%` 下显示成 `108.8%`，
六个字符，宽度 6 的列上就是 `###`。只量文本格的实现看不见这一类
（实测：`利润表.xlsx` 里**唯一真的显示不出来的就是这种格**）。数字格式解析不了的
（条件/颜色/科学计数/日期/带引号字面量）**如实报 `unmeasured_cells`，不猜**。

**报告里 `widths_measured` 逐列给出「量到什么 / 需要多少 / 现在多少 / 结论」，一列没改也给。**
`changes: []` 与「`--autofit` 根本没生效」是同一串字节，调用方分不出来就会自己动手 ——
实测有过：模型连跑两次 `--autofit` 拿到两个空 changes，然后绕过技能硬写了五个宽度，
比它自己算出的需求还大，**把表挤到了两页**。⚠️ **列宽不是越大越好** —— 加宽会把右边的列
挤到下一页，而第 1 页看起来是完整的。`xlsx_pdf.py` 现在会报 `columns_off_first_page`。

**渲染那一侧也查了三件「看起来没问题」的事**（`xlsx_pdf.py`）：`blank_pages`（整页没墨）·
`columns_off_first_page`（表被横向切开）· **`hash_marked_cells`（有数字渲染成 `###`）**。
第三件是 B9 抓的：**一页、有墨、没切开，而 `108.8%` 不在纸上**。
`hash_marked_columns` 用与 `--autofit` 同一套显示宽度点名是哪一列（实测阈值：
六个字符的数字在宽度 6 上是 `###`、7 上正常）；**表里真的写着 `###` 时不下结论**，
如实报 `null` + 说明。`--png` 目录里上一次渲染留下的 `page-NNN.png` 会被删掉并列在
`stale_images_removed`（只动这个脚本自己写的文件名）。
...
```bash
python3 scripts/xlsx_pdf.py --in book.xlsx --out 输出/book.pdf --png 输出/pages --dpi 150
```

⚠️ **PDF 和 `--png` 的图必须落在工作区内**（相对路径，如上）。写到 `/tmp` 或任何
工作区外的绝对路径，产物面板扫不到、回复里的内联图也读不到 —— **文件在，用户看不见**，
而这条唯一的通道就白开了。

**纯中间文件**（下一条命令马上吃掉的临时 JSON）不受这条约束——别拿它堆满用户的工作区；
但也别写死 `/tmp`：Windows 上它落到系统盘根目录 `C:\tmp`，而 `--out` **会替你把父目录
建出来** —— 所以它不会报错，只是在别人的 C 盘根上留垃圾，同名文件还会被并发会话互相覆盖。
用 `tempfile`。（2026-08-16 实测更正：此前这里写「不存在就直接失败」是错的。）

三件事它拒绝而不是糊弄过去:**整页没有墨的 PDF**（LibreOffice 对空表照样退出 0 并产出
白页，把它当预览交回去比报错更糟——那看起来就像数据没了）· **中文渲染成方块（豆腐块）**
（见下）· **没算过的 workbook**（公式格渲染成**空**，图片本身没法告诉你哪里不对，所以先警告）。

**豆腐块守卫**：机器上没有覆盖这些汉字的字体时，LibreOffice **照样退出 0、写出一份合法 PDF**，
...
**把这句原样转述给用户**，不要软化成「预览可能有字体问题」。报告里 `tofu: false` + `tofu_measure` 是**量过的结论**；
带 `tofu_note` 的 `false`/`null` 是「没东西可量」，不是量过没事；要渲染的表（含表名）里没有中文时 `tofu: false` 且不量（零影响）。
确认方框可接受时加 `--allow-tofu`，会写出并在报告里给 `tofu: true` + `tofu_warning`。
修法在机器不在文件：Linux 装 `fonts-noto-cjk`，Windows 装任一 CJK 字体，macOS 由技能自动给
soffice 传 `SAL_USE_VCLPLUGIN=osx`（自己设了这个变量会覆盖它）。

`--png` 需要 pypdfium2（Apache-2.0，pdf 技能的依赖）。它**不是**本技能声明的依赖（为几个
预览附加功能给整个技能挂红徽标不合理），所以缺它时：PDF 照出，空白页与豆腐块检查降级为**明说的
缺口**（`tofu: null`），而 `--png` **直接报错** —— 你明确要了图，不产图却安静退出才是这份文件最该避免的失败。
...
- **真的没有入口的两件事**（实测，2026-08-05）：**创建合并单元格** —— 合并能读、
  `--autofit` 也会正确地不把横跨多列的标题算进第一列，但没有任何脚本能**建**一个合并区；
  **新建工作表** —— `xlsx_convert.py --from` 每次产的是一个只有一张表的**全新** workbook，
  所以跨表公式只能写在本来就有多张表的文件上（财务色彩规范里的**绿**因此在单表文件上演示不出来）。
  ⚠️ 这一条在 S3 之前列的是另外四件事（条件格式 / 图表 / 数字格式 / 冻结窗格的创建），
  而 S3 把它们全做了却没回头改这里，于是它当了一段时间的假话。
  **SKILL.md 是模型判断自己会不会做某件事的唯一依据**，一句过期的「不做」会让它
  绕开一个已经能用的脚本 —— 现在由 `check-docs.ts` §12 对着 `capabilities.json` 逐行对账。

  **用户要这两件事时怎么办**（2026-08-16 实测两次都被自己用 openpyxl 做掉了，一次说了、
  一次一个字没说）：**先说这里没有入口，再说自己动手的代价，然后让用户决定** ——
  - `openpyxl.load_workbook(f) → save()` 会**重建整个包**。本次实测这份文件没丢 part
    （图表逐字节还在），但**它丢掉了全部缓存值**，且这份文件本来就没有
    `metadata.xml` / `<ignoredErrors>` 这类 openpyxl 不认识的东西 —— **换一份文件就不成立**。
  - `ws.insert_rows()` **不调整任何公式引用**：插一行标题之后 `E3` 里还写着 `=B2/D2-1`，
    而 B2 现在是表头文字。实测 17 条公式全部错位。
  - ⇒ **真动手了就必须跑 `xlsx_recalc.py` 或 `xlsx_audit.py` 复验**（实测那份错位文件
    重算会报 `error: 12`，逐条给出 `#VALUE!` 和是哪个格）。**别只报告「已完成」。**
- **写文件的脚本都会报 `replaced_existing`**（写之前问，写之后永远是真）。
  `--out == --in` 是唯一被拒绝的情况；**不同路径、不同扩展名、别人的字节一概拦不住**，
  而在 2026-08-16 之前这个技能对此一个字都没有。**刻意不做成拒绝**：重跑一次覆盖自己
  上次的产物是常态，需要 `--force` 的工具会被人一律加上 `--force`。
- **`xlsx_audit.py` 只解析引用、不求值**（这条仍然成立，也是有意的）：要真正的数字走
  `xlsx_recalc.py`（双引擎互校）。一旦审计器开始算数，每个算错的地方都会变成一个自信的错数。
