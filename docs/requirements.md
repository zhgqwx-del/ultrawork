...
- [x] 办公 CLI 连接器 Phase 2：钉钉 dws — 双源 pin 安装 + 官方技能 materialize + `authenticated` 探针 + 第六态「白名单未开通」引导（管理员姓名+后台深链）+ dingtalk-assistant 薄路由技能；泛化债还清（child 槽 HashMap 化/hook generation 按 id 拆）（ADR-043）✅ 2026-07-07（真机全流程验收；Phase 3 企微待做）
