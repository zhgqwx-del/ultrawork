// builtin-deckcraft-ui.e2e.ts — end-to-end UI proof for the bundled deckcraft skill,
// driving the REAL React app (Chrome + Vite + real opencode). deckcraft is the default
// "make a PPT" skill (ppt-master was removed from the bundle in P3, ADR-061 /
// discussions/043 §18.5 — it survives only as a curated catalog entry, not a builtin).
//   1. the Settings → Skills built-in tab lists /deckcraft (discovered from the
...
//
// Run:  cd packages/client/desktop && bun run --bun e2e/builtin-deckcraft-ui.e2e.ts
...
const tmp = mkdtempSync(join(tmpdir(), "deck-ui-"))
...
// Controlled host picture: everything present EXCEPT the two version/import probes
// (python3.10+ / python-pptx) — exactly what a stock macOS (python 3.9, no pip libs)
// looks like. deckcraft also needs chrome-or-edge (present) and node (present,
// OPTIONAL), so its only missing deps under this fixture are the two python probes.
const DEPS = [
...
try {
  if (!existsSync(join(SRC, "deckcraft/SKILL.md"))) throw new Error("skills/builtin/deckcraft missing — run fetch-builtin-skills.ts")
  ...
  // 1. deckcraft card discovered through real opencode + api-client
  const card = page.locator("div.rounded-lg", { hasText: "/deckcraft" }).first()
  await card.waitFor({ state: "visible", timeout: 15000 })
  checks.push("built-in tab lists /deckcraft (real GET /skill through the app) ✓")
  const cardText = (await card.innerText()).replace(/\s+/g, " ")
  if (!/PPTX/i.test(cardText)) throw new Error(`deckcraft card lost its description: ${cardText.slice(0, 160)}`)
  checks.push("card carries the description (PPTX) ✓")

  // 2. dep badge shows exactly the two probed gaps + the guide button
  if (!/(Missing|缺少)/.test(cardText)) throw new Error(`no missing badge on deckcraft card: ${cardText.slice(0, 200)}`)
  ...
  if (!prefilled.includes("deckcraft")) throw new Error(`composer not prefilled with skill name: "${prefilled.slice(0, 120)}"`)
  ...
  if (!prefilled.includes(join("skills", "builtin", "deckcraft"))) throw new Error(`prefill lost the SKILL.md location: "${prefilled.slice(0, 200)}"`)
  ...
  verdict = "PASS ✅ — real React UI lists the bundled deckcraft, renders the probed dep gap with a guide button, and hands off a complete install prompt to the Home composer."
} catch (e) {
