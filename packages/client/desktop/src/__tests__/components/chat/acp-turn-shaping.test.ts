...
import rawCodexFixture from "../../fixtures/acp-codex-turn.json"
...
const codexFixture = rawCodexFixture as ShapedEvent[]
...
// ADR-060 gate for codex (feat/team-agent-codex-cli): a REAL codex turn via the
// @agentclientprotocol/codex-acp bridge, captured through the same shaping
// pipeline (fixtures/acp-codex-turn.json, generated by spike-codex.ts in the
// default `agent` mode). Must split process/answer identically to claude/hermes
// — proof that codex-via-bridge is a plain ACP agent needing no bespoke shaping.
// (Codex opens with a preamble text before its tools, so the leading assistant
// message is a tool-calls step, not the answer — the same three invariants hold.)
describe("ACP turn shaping → buildTurnModel (real codex fixture)", () => {
  it("emits every delta after a part.updated created its part", () => {
    const seen = new Set<string>()
    for (const event of codexFixture) {
      if (event.type === "message.part.updated") seen.add(event.properties.part.id)
      if (event.type === "message.part.delta") {
        expect(seen.has(event.properties.partID)).toBe(true)
      }
  ...
  it("splits the turn into process (tool steps) and a text-only answer", () => {
    const messages = replay(codexFixture)
    expect(messages[0].info.role).toBe("user")
    const assistant = messages.filter((m) => m.info.role === "assistant")
    expect(assistant.length).toBeGreaterThanOrEqual(2)
  ...
  it("ends the turn with a terminal finish (spinner stops)", () => {
    const assistant = replay(codexFixture).filter((m) => m.info.role === "assistant")
    const lastInfo = assistant[assistant.length - 1].info
    expect(lastInfo.finish).toBeTruthy()
    expect(lastInfo.finish).not.toBe("tool-calls")
    for (const msg of assistant.slice(0, -1)) {
