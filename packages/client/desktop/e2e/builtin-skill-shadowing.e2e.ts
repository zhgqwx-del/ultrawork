...
//      "ppt-master" and the winner is arbitrary (logged, not asserted).
...
//      under skills/ppt-master) is served immediately.
...
// Real upstream SKILL.md (the whole 12k-file tree is irrelevant to discovery).
const REAL_SKILL_MD = join(REPO, "skills/builtin/ppt-master/SKILL.md")
const PW = "shadow-pw"
const OC = 4302
...
const builtinDir = join(skillsRoot, "builtin/ppt-master")
const userDir = join(skillsRoot, "ppt-master")
...
async function getPptMaster(): Promise<{ entries: number; location: string; description: string }> {
  const skills = await (await fetch(`${base}/skill`, { headers: WH })).json() as any[]
  const hits = (Array.isArray(skills) ? skills : []).filter((s) => s?.name === "ppt-master")
  return {
    entries: hits.length,
...
try {
  ...
  await softRefresh()
  const afterPrune = await getPptMaster()
  ...
  installBuiltin()
  await softRefresh()
  const afterRestore = await getPptMaster()
