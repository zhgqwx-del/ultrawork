...
⚠️ **更正一处此前反复写错的话**：不是「CI 从未跑过这个分支」——
`gh run list` 查得清清楚楚，本分支 2026-08-05 在 `af9cb455` 上**跑过并全绿**（run 30992724500）。
真正没被跑过的是 **L4 阶段的两个 commit**：`0eaab38b`（第一、二节）与本刀 `f63dc435`，
它们 2026-08-16 之前一直躺在本地。**「分支没跑过」和「这两刀没跑过」不是一回事**，
前者把未知说大了，而说大的未知和说小的未知一样会误导判断。三平台仍是本机验不了的部分。
...
### 三十七、CI 第一次跑到 L4 这两刀：6 个 job 红，三条都只在别的机器上成立（2026-08-16）

`f63dc435` 推上去（run 31954958283）：**docs / node ×3 / rust ×3 全绿，
office skills 2 红、L3 corpus 3 红**。三条根因互不相同，**没有一条能在本机看见**。

#### 一、L3 三平台同时红：pymupdf 1.28 往 **stdout** 打了一行字，把结果协议冲掉了

现象：`C14 健康 pdf 过环 → gate_limit ['P5 判不了：L2 子进程 rc=0: nocap(ValueError)']`。

根因：**pymupdf 1.28 起 `import fitz` 会打印**
「warning: The `fitz` API is deprecated and will be removed in future.」——
**打到 stdout**。L2 的 P5 正是 `import fitz`，而 L2 worker 用 **stdout 传 JSON**
⇒ `json.loads` 失败 ⇒ 每一份健康 pdf 都被记成「判不了」，**而退出码是 0**。
本机 pymupdf **1.27.2.3 一个字都不打**（CI 装的是最新，本机是几个月前装的）
⇒ 本机 31 项全绿。**又一次「只在别的机器上现形」，而这次差别是一个次版本号。**

⚠️ **报出来的那句话还指错了地方**：消息里引用的是 stderr 的最后一行，
而那一行永远是 `self_cap_memory()` 的 `cap=`/`nocap()`，与失败原因毫无关系 ——
**「rc=0 但结果读不出来」此前和「rc≠0」塌陷成同一句话**（MEMORY 里记着的那条
「超时与撞内存上限共用一个 GATE 标记」是同一族的第二例）。

**修法两道，防的不是同一件事**：① worker 把 stdout 让出来（检查函数与它们 import
的任何东西打的字一律去 stderr）；② 结果前加 `__L3_L2_JSON__` 标记，读的一侧只取标记
之后的那一段 —— 万一有 C 扩展直接往 fd 1 写，①拦不住而②仍然读得对。
诊断同刀改掉：rc=0 却读不出来时明说是哪一种（没有标记 / 标记后不是 JSON）并附
stdout 前 160 字。

**控制臂 C31（新增，30 → 31 项）刻意不绑定库、也不绑定版本**：用 `sitecustomize.py`
让子进程一启动就往 stdout 打字，等价于「某个被 import 的东西打了字」。
**实测两个方向都钉住了**：撤回修复的副本上 C31 报出与 CI **同一形状**的
`rc=0: nocap(ValueError)`（而 C14 在本机 1.27 下仍然是绿的 —— 这正是本机看不见它的原因）；
修复后在**本机 1.27 与 CI 同版本 1.28.2 两套解释器上各跑一遍，都是 31/0**。

#### 二、ubuntu 的 pdf 门禁：控制臂选到了同一个字面，于是它「没响」

`create-takes-the-first-face-that-registers` 期望 G8 响，实际没响。
它本来就带一个逃生舱（机器上**一个 CJK 面都没有**时 `ControlUnavailable` 跳过并留名），
但 ubuntu 上的情况是**有 CJK 字体、却没有第二个字重** ⇒ 把选择规则反转过来
**选中的还是同一个面**，两条臂的产物逐字相同。
⇒ **「有 CJK 字体」不等于「有第二个字重」**，而这从外面看和「护栏坏了」一模一样。
修法：控制臂跑完比对两条臂真正选中的 `typeface`，**相同就跳过并把面名说出来**。
本机（有 Songti.ttc 八个面）G8 仍然照响，72/0 未变 —— 逃生舱没有把控制臂变成摆设。

#### 三、docx 的 D7 控制臂：**ubuntu 与 Windows 都顶不出带外**，而我第一版判据挡错了地方

`LIVE: from_md restores the <w:docGrid> line grid` 期望 D7 响，实际没响。

⚠️ **先更正我自己读错的两处**：① 第一轮我写成「Windows 专有」——**是错的**，
ubuntu 同样红；第一轮 ubuntu 的 docx 步骤**根本没跑到**（pdf 门禁先红把 job 截断了），
我把「没跑」读成了「绿」。**一个没执行的步骤和一个通过的步骤，在 job 摘要里长得一样。**
② 第一版修法判「注入前后**绝对行高**变了没有，差 < 0.5pt 就跳过」——**没拦住**，
因为那两台上行高确实变了一点，只是**不够把比值顶出 D7 的带**。
⇒ **控制臂的判据必须就是断言自己的判据，不能是它的近亲。**

修法：控制臂跑完直接看**注入后的比值是否仍落在 D7 的带内**（`D7_BAND` 现在是
断言与控制臂**共用的一组数**，各写一份的话改了一处就会让控制臂沉默地失去意义），
落在带内即 `ControlUnavailable` 跳过，并把**四个数字**（两条臂的 single/spaced 与两个比值）
写进 skip 原因 —— 下一轮 CI 直接读得到 ubuntu/Windows 的实测值。
macOS 上同一份文档实测 23.1 → 40.6pt/行、比值 **2.28**，D7 照响，本机 215/0 未变。

**下一轮 CI 把那两台的数字送回来了**（这正是把数字写进 skip 原因的目的）：
**ubuntu** 不注入 19.8pt/行、注入后 20.3pt（比值 1.303 → **1.336**）·
**Windows** 19.2 → 20.3pt（1.297 → **1.372**）· 对照 **macOS 23.1 → 40.6pt（1.30 → 2.28）**。
⇒ 那两台的 LibreOffice **几乎不受这条线格影响**（+0.5 / +1.1pt），
而我第一版的判据是「差 < 0.5pt 就跳过」—— **ubuntu 那台正好是 0.5**，
差一点就挡住了，靠的是运气不是判据。**近亲判据的危险就在这里：它有时候恰好对。**

⇒ 老实说清这个修法的代价：**D7 的负向控制此后只在 macOS 上真正跑**。
产品护栏三平台都在（生成器不再写 `docGrid`），但「护栏坏了会不会被抓到」这一格，
ubuntu/Windows 上是空的，且**跳过会打印在 skip 清单里**，不会假装它跑过。

#### 四、这一轮最该记住的

**「本机全绿」这次连自己的复述都骗过了**：我在 §三十六 写下「本机门禁全绿」时，
那句话是真的 —— 它只是**不蕴含**「在别的机器上也绿」。三条红里，
一条是上游次版本号变了、两条是控制臂在那台机器上没有被测对象。
⇒ **CI 抓到的第 7~9 个「本机抓不到」的问题**（前六个记在
`office-skills-s3-s6-lessons.md`），而这次三条全部**发生在「本机全绿」之后**，
与那六条的形状完全一致。

#### 五、第三轮只剩一个红，而它是我这一刀的门禁第一次在 Windows 上跑

`office skills (windows-latest)`：**pptx-edit 门禁 14/1**，红的是
`LIVE: pptx_read without the UTF-8 reconfigure` 这条控制臂 —— `unexpectedly fired: ['N1']`。

不是缺陷，是**级联清单过期**：那条控制臂在敌对代码页的机器上本就会打出 `C1/V0/X1`
（脚本第一个中文字就死，所有读它输出的断言跟着响），而 **N1 是 2026-08-16 才加的**，
清单写在它出生之前。一个**死掉的脚本当然也不打 `[unread]` 标记**。
修法：非 UTF-8 主机的期望集合补上 `N1`，并在注里写清它为什么在里面。

**这一条本来在本机就能抓到，我没跑那一步。** 门禁自己早就有模拟 Windows 的手段
（`ANSI_ENV` / `PYTHONIOENCODING=cp1252`，S6 就是靠它把一个 Windows 专有缺陷变成
三平台可复现的），而 `default_captured_encoding()` 读的是**子进程实际拿到的编码** ——
**在父进程设 `PYTHONIOENCODING=cp1252`，整个门禁就会走非 UTF-8 分支**。
补跑后本机 **15/0（captured-stdout encoding: cp1252）**，与 Windows 同一格。
⇒ 顺手把 docx / xlsx / pdf / L2 / deckcraft / L3 selftest 都在 cp1252 下过了一遍，
把「等 CI 告诉我」换成「先在本机问同一个问题」。

#### 六、那一遍 cp1252 当场又抓到一条，而且是我这一刀自己埋的

`deckcraft-selftest` 在 cp1252 下**崩在自己的 `print` 上**（`UnicodeEncodeError`）：
`scripts/deckcraft-selftest.py` **从来没有 UTF-8 守卫**，而我新加的三条断言
（§三十六 缺陷 30 的正向断言）**第一次把中文带进了它的输出** —— needle 就是
`视觉审查`。同目录另两个 deckcraft 门禁（contrast / validate）同样缺。

⚠️ **危险的地方在于它不在 CI 里**：deckcraft 的 python 门禁只在真机跑
（053 §4.4 记着这条），所以**只有 Windows 真机会撞上**，而 Windows 真机验收本来就欠着 ——
等于往那批欠账里**新埋了一颗雷，还是我自己埋的**。
修法：三个门禁各补上与其它十二个门禁同样的两行；补完在 cp1252 下
**52/0 · 52/0 · 96/0**。

**产品侧核过是干净的**：`export_deck.py` 用的是共享模块
`console_encoding.configure_utf8_stdio()`，我新加的那三行中文提醒在 Windows 上不受影响 ——
这也再次印证 §三十六·六里那条「我判错的第 4 次」（当时报的就是「export_deck 无 UTF-8 守卫」）。

⇒ **一条更普遍的**：**给一个既有脚本加中文输出，就是给它加一条 Windows 依赖**。
这个仓库里「打中文的脚本」与「有 UTF-8 守卫的脚本」本该是同一个集合，
而它此前不是；判断标准不是「这是不是新脚本」，是「这一刀之后它会不会打非 ASCII」。
