...
#### 三、Windows 的 docx 门禁：注入 `<w:docGrid>` 之后纸上没变化

`LIVE: from_md restores the <w:docGrid> line grid` 期望 D7 响，实际没响 ——
即在 Windows 的字体度量下**那条线格不产生量化**，控制臂改不动纸。
同一形状、第三次：**缺陷在这台机器上不成立** ≠ **护栏坏了**。
修法与②同规矩：控制臂先证明自己真的把纸改了（比较注入前后的**绝对行高**，
差 < 0.5pt 即判定复刻不出来），跳过并**把两组数字写进 skip 原因**——
下一次 CI 就能直接读到 Windows 上的实测值。判据取绝对行高而不是 ratio：
线格会把两条臂一起缩放，ratio 对这种情况是瞎的（这条 `measure_spacing` 的注释里早写着）。
docx 门禁本机 215/0 未变，D7 仍然照响。
