...
| [028-im-channel-qr-and-brand-icons.md](./028-im-channel-qr-and-brand-icons.md) | 消息渠道扫码接入（钉钉/飞书/企微）+ 渠道品牌 icon — 三家官方设备流「扫码建机器人」机制源码级尽调（飞书 accounts registration/PersonalAgent+Addons · 钉钉 app/registration 三步 · 企微 ai/qc + 智能机器人长连接 101463）+ 收发链路可行性核实 + icon 素材/授权结论（**不用自画**：官方资源+开源库组合，Dify/LangBot 先例）+ 分期方案（icon→B0 拆分→B1 钉钉扫码化→B2 企微→B3 飞书）；对抗审查修订（QR 骨架必须改 gateway 后台轮询+缓存〔设备流 secret 一次性〕、secret 0600+masking、企微渠道 bot 严禁复用 CLI bot、真机实拍前置清单、D1-D6 待拍板） | 2026-07-07 | 调研记录 + 讨论中 |
