...
## 24. 重连后补拉数据：合并，不要重新 seed（ADR-072）

§22 讲的是**状态**（转圈、busy 标记）重连后从服务端重新求值。**正文/列表数据**也丢，但不能照抄那个写法 —— 状态可以整份覆盖，列表不行：列表往往有分页窗口、乐观条目、以及被 revert 掉的历史，整份覆盖等于把用户正在看的东西抽走。

**规则**：重连补拉走一条「只合并、不重置」的路径，与初始加载分开。

```ts
// ✅ 就地合并：位置不变，只让内容变得更全
setMessages((prev) => mergeSnapshotInPlace(prev, snapshot))

// ❌ 复用初始加载：setMessages([]) + 重置 cursor/hasMore/turnStart
//    用户已加载的更早历史会消失，视口跳动
```

合并函数的四条不变式，每条都对应一个具体的坏结果：

| 不变式 | 不遵守会怎样 |
|---|---|
| 已有条目**保持位置**，只升级内容（并刷新 `info` 之类的元数据） | 重排 = 用户正在读的那段跑掉了 |
| 快照独有的条目插到**最近一条双方都有的条目之后** | 一律追加到尾部 = 顺序错乱 |
| **比本地最早一条还老的快照条目直接丢弃**，不前插 | 前插 = 打乱分页窗口，`cursor` 之后再也对不上 |
| **不删除**本地有而快照没有的条目 | 删除 = 抹掉乐观条目 / 复活或误杀已 revert 的轮次 |
