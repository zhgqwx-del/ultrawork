...
- `check_skill_dependencies`（async；PATH 探测 + `run_python_feature_probe` python 内探针〔python3.10+ 版本门/python-pptx，四防御见 gotchas §10〕，复用 rich_path）；`ensure_builtin_skills`/`find_builtin_source`/`builtin_needs_refresh`/`install_builtin_tree`（首启拷贝 `skills/builtin/` → `~/.config/ultrawork/skills/builtin`，staging+rename 原子交换，sentinel 控刷新，ADR-032/040）
