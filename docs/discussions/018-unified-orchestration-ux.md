...
> **状态**：✅ 已实施（2026-06-12，A-1~A-4 + 议题 B 随 A 落地，分支 `feat/agent-os-phase0` M1-M6；GUI 走查 10/10）。存量迁移实测：vendor PATCH 不支持改 parentID → 走侧栏补显 fallback（§2.4 预案）。
> **日期**：2026-06-12
...
### 2.4 拍板结果（✅ 2026-06-12 四项全拍板）

| # | 问题 | 拍板 |
|---|------|------|
| A-1 | Team 会话进侧栏的形态 | ✅ **混排 + 徽标**（最贴「统一交互」诉求） |
| A-2 | 模式出生锁定 vs 中途升级 | ✅ **出生锁定**（ACP 技术约束 + 心智简单） |
| A-3 | `/orchestration` 去留 | ✅ **留流水线、去 Team tab**（代码驱动面受众不同，保持独立） |
| A-4 | Leader 隐藏父机制 | ✅ **Leader 转 root**（Team 身份由注册表标记；twin/委派子会话隐藏机制**不变**） |

> A-4 是唯一触及「逻辑」的点（一行级：创建 leader 时不传 parentSessionId），其余纯 UI。
> 兼容：存量（挂隐藏父的）Team 会话——实施时先试 `PATCH /session` 改 parentID 迁移；vendor 不支持则侧栏数据源对注册表 legacy 条目补显（量小，实施时定）。
> 附带收益：leader 转 root 后 opencode **自动标题**生效——议题 B 的「列表摘要标题」大半自动解决。
>
> **节奏与分支（同日拍板）**：先实施 A（surface 重排）→ B 随 A 的新施工面一并做；**018 在 `feat/agent-os-phase0` 分支继续，整体完成后一次合入 main**。
