...
**状态**: Accepted（架构决策）· **阶段3 第一批 + 第二批 + 第三批已落地（2026-06-12）**——实现章节五项全实现：①原语层 + ④代码驱动 Pipeline + ②agent 驱动 delegate（`delegate-mcp` stdio shim + 双注入路径）+ ⑤Fan-out（DAG 执行器 + worktree 隔离）+ ③UI（独立路由 + 主对话 delegate 卡片/dock）。宿主 = ACP sidecar :4099，`/orchestration/*`。**第三批（[017](../discussions/017-team-page-agent-driven-orchestration.md)）确立 D-3 opt-in 的产品形态 = Team 页**（`/orchestration` 两 tab）；委派是 Leader 会话的默认行为（system 提示驱动），普通会话物理隔离（全部非 Leader opencode prompt 恒 deny `orchestrator_*`），「编排模式」Settings 开关移除。**surface 形态后续演进**：018 把 Team 融入主聊天流、`/orchestration` 回归**纯流水线页**（去 Team tab）；019 收纳流水线 UI 后**「自动化」nav 入口暂时下线**（[019](../discussions/019-pipeline-surface-form.md) §7，路由+代码全保留可深链，orchestrator 后端 Team delegate 共用未死）。落地偏差备注见下方「实现章节」。**已整体合 main**（2026-06-13，merge `232c8fa`）。
**日期**: 2026-06-08
