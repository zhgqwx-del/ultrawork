...
- **会话↔agent 绑定只存前端 localStorage**（`uw.acp.sessionAgents`，档1 设计）：清 WebView 数据/换设备后绑定丢失 → 前端把 ACP 会话当 opencode 会话（历史「消失」但 sidecar 盘上数据还在、AgentSelector 回落 OpenCode）。Tauri 正常重启不受影响（WebView localStorage 持久）。阶段2 connector 应把绑定持久化到 sidecar（`GET /acp/session/:id` 已可反查）。
