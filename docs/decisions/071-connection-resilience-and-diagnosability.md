...
- **✅ F1b：断流期间的正文补拉 —— 已于 2026-07-30 实现（ADR-072）。** 量化见 discussions/058：断流落在回合内完全自愈（缺失 0），**跨越回合结束时丢失起点之后 97.7% 正文且永不自愈**（300 marker 只剩 7），sidecar 崩溃必然属于后者。修法未碰那条危险的分页合并 —— 该场景下会话已 idle，而初始加载的注释写明 idle 可完全信任服务端快照；改为「重连 + idle 时就地合并」（`mergeSnapshotInPlace`：保持位置、只升级正文更长的 part、不前插更老的分页、不删除任何消息）。
