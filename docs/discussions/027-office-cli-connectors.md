...
> 状态：**三阶段全部实现并真机全流程验收，范式收官**（飞书 2026-07-06 / 钉钉 2026-07-07 / 企微 2026-07-07，分支 `feat/office-cli-connectors`，ADR-043 为决策记录）——§6 待验证清单全部实测回填（lark 6 坑 + dws 与 lark 多轴相反 + wecom 第三套契约，SSOT 固化 gotchas §14 三段）；三项泛化（connector registry / probe 与阻塞流骨架去重 / e2e mock 工厂化）随 Phase 3 兑现（ADR-043 后果节）
> 日期：2026-07-06
...
> ⚠️ 本表是 2026-07 初上网复核快照，技能数以 §6 实测为准（lark 实测 27 个内嵌 / wecom 实测 9 个 repo 目录），其余维度亦以 §6 各段实拍回填为权威。
...
- [x] **官方技能 = 9 个（非 §1 表的 12）**，repo `skills/` 目录（wecomcli-{contact,doc,meeting,msg,schedule,sheet,smartpage,smartsheet,todo}，208KB/3.6k 行，frontmatter 标准）；官方分发 = `npx skills add WeComTeam/wecom-cli`（**拉 repo HEAD、不 pin、依赖第三方 skills 工具**）；技能既不内嵌二进制（lark 式）也无独立 zip 工件（dws 式）→ **vendored（用户已确认 2026-07-07）**：官方 9 技能收进 builtin zip 作 wecom-assistant 的 `references/official/`（MIT，SKILL.md→INDEX.md 防嵌套误扫）。**落地修正**：快照取 main HEAD `9eb7898b` 而非 0.1.9 gitHead（后者只有 7 技能且 doc 路由与新技能矛盾；工具可用性服务端动态下发、与 CLI 版本解耦）——取舍与「整树单一 commit、git archive 双向核实」维护规则见 `_ORIGIN.md`。
...
- [x] ~~待真机验收实拍~~ **真机全流程验收通过（2026-07-07，用户扫码全链走通）**：① `auth show` 已授权实拍 = pretty JSON `{"create_time":…,"id":"aibk…"}`（**id 字符串**）、`--auth-status` = `authorized` 纯文本、凭证三件套 0600 落盘、无孤儿进程；② 用户企业（深圳梅克斯科技）= **≤10 人全量档**（6 品类 ~47 工具全开通：contact 1 / doc 23 / meeting 5 / msg 4 / schedule 8 / todo 6）；③ 普通成员扫码即建机器人成立（无管理员审批环节）；`--schema` 实拍输出完整参数 JSON Schema、真调 contact 域实证双层信封；agent 端到端 `tool_search→skill(wecom-assistant)→read(官方 todo INDEX)→bash(auth show)→bash(todo get_todo_list×2)` 返真实数据。**遗留观察项**：重授权 rollback 清旧凭证（UI 已连接态无重授权按钮、暴露面极小，gotchas §14 已记）；④ Windows 运行时（含 PowerShell JSON 参数传递）随 Windows 批次（ADR-037 同类）。
