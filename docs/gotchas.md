...
- **ACP 真机走查的 agents.json 不随 XDG 隔离（2026-06-13 实测，测试隔离铁律）**：`agents-config.ts` 的 `CONFIG_DIR` **硬编码** `homedir()/.config/ultrawork`，**忽略 `XDG_CONFIG_HOME`**——即便用 `XDG_CONFIG_HOME=/tmp` 起隔离 acp sidecar，经 GUI/`PUT /acp/agents/:id` 增删 agent 仍读写**真实** agents.json（而 opencode 数据/ACP 会话经 `XDG_DATA_HOME` 正常隔离到 /tmp）。→ 凡走查会动 agent 注册的，**必须先 `cp -a` 备份 + 记 md5 基线，走查后还原并校验 md5**（防把测试 agent 留在真实配置）。凭证用 `OPENCODE_SERVER_PASSWORD`/`ULTRAWORK_SIDECAR_PASSWORD` env 覆盖，避免碰真实 sidecar-auth.json。
