...
- **关窗口不退出 / 托盘（ADR-074，`src-tauri/src/background.rs`）**：`close_action(os, tray_ready, boot_failed)` 纯函数决定 Hide/Quit；`install_tray()` 在 `setup()` 试建一次（`catch_unwind`，Linux 缺库是 panic）；`restore_main_window()` 供 single-instance / macOS `Reopen` / 托盘共用；`set_tray_labels` 命令由 renderer `src/lib/tray-labels.ts`（`I18nProvider` 的 effect）推送应用语言文案。mac 菜单栏模板图 `icons/tray-template@2x.png`（占位）。
