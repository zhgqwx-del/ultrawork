...
- **关窗口不退出 / 托盘（ADR-074，`src-tauri/src/background.rs`）**：`close_action(os, tray_ready, boot_failed)` 纯函数决定 Hide/Quit；`install_tray()` 在 `setup()` 试建一次（`catch_unwind`，Linux 缺库是 panic）；`restore_main_window()` 供 single-instance / macOS `Reopen` / 托盘共用；`set_tray_labels` 命令由 renderer `src/lib/tray-labels.ts`（`I18nProvider` 的 effect）推送应用语言文案。mac 菜单栏模板图 `icons/tray-template@2x.png`（占位）。真机门禁 `scripts/verify-close-to-background-macos.sh`（AX 驱动原生窗口，testing §14）。
...
- `skill-creator`/`skill-installer`/`pdf`/`markdown-exporter`（上游 Apache-2.0）——由 `scripts/fetch-builtin-skills.ts` 同步+打补丁（支持按名过滤/post-patch），勿手改；**`pptx-edit`**（自写，原名 `doc-edit`；059 S6 瘦身：`docx_*`/`xlsx_*` 四个脚本被专用技能取代后删除，只留 .pptx 就地读改——**唯一没有专用技能覆盖的一块**。刻意薄：只有全局替换文字 + 追加一页两件事，SKILL.md 的「限制」逐条实测：表格/组合形状里的文字**读不出也替换不到**、跨 run 短语匹配为 0、而 `replacements: N` **不会报「还有几处我看不见」**）、**`xlsx`**（自写，openpyxl 读 + 自建 OOXML 底座外科式写，15/15 能力零 pending，059 S3）、**`docx`**（自写，**纯 lxml、刻意不用 python-docx**；核心是「一句话不是一个 run」的段落字符流跨 run 替换，**19/19 零 pending**，判据 = CI 的 `--no-pending pdf xlsx docx`；修订族纯 Python 接受/拒绝，五种形态 + `remaining` 自检契约（**读与解析覆盖全部 header/footer，写仍 opt-in** —— `docxcommon.text_parts()`，gotchas §21.2 ㊳）；版式族 = 页眉页脚**四样+第五样开关** · 目录是**域**且页码故意不写 · 图片尺寸走 **EMU** · 字体巡检先走**样式链**再写；W12 改样式会**重绘整篇**故要 `--overwrite`、W4 从 Markdown 生成**映射不了的构造一律带行号点名**；**W18 `docx_diff.py`** 一处差异 = 一段可见文字变了 + 白名单，噪声逐类报计数不计为差异，产物是 **redline docx**，判据是闭环（接受本次修订 ⇒ 等于 B / 拒绝 ⇒ 等于 A）；**W19 `docx_table.py`** 只 3 种预设，每种都由可测量属性定义（`w:sz` 八分之一磅 · `w:tblHeader` 渲染层实测 · 列宽按 **CJK 显示宽度 130 dxa/单位**），指纹从产出文件读回来两两必须不同，059 S4 §六·补五/补六/补七）、**`deckcraft`**（自写，HTML-first PPT，**做 PPT 的默认技能**：Research→大纲 evidence 门禁→spec_lock→并行 HTML→溢出探针→HTML/PDF/图片型 pptx/可编辑 pptx；P3 起放宽接管「做PPT/生成PPT/演示文稿/幻灯片/slides/deck」全触发面，ADR-061 / discussions/043 §18.5；**ADR-068 形式丰富度轴**：风格库 `references/design-styles/` **10 套**〔`_index.md` 带温度列，pick_variants 解析〕· 版式库 `assets/templates/layouts/` **20 个**〔一版式一 `Sxx.html` + `_index.md` 选型索引，**registry SSOT = 目录内容，新增即生效零改码**〕· `scripts/pick_variants.py` 确定性候选打乱 · 12 个骨相 token + `--c-head`/`--font-display` · 门禁 O10/O11/W3/W4/W5 · E4 风格级白名单〔shadow+gradient〕）、`feishu-assistant`（自写，飞书 lark-cli 薄路由，ADR-043）、`dingtalk-assistant`（自写，钉钉 dws 薄路由——路由到连接器 materialize 的官方 mono 文档，ADR-043）、`wecom-assistant`（自写薄路由 + `references/official/` vendored 官方 9 技能单一 commit 快照〔SKILL.md→INDEX.md 防嵌套误扫，见 `_ORIGIN.md`〕，ADR-043）可直接编辑（改完重打 zip）。**`ppt-master`（上游 MIT，ADR-040）P3 起不再内置**——整树已删，仅作 `INSTALLABLE_SKILLS` curated 长尾退路（美化已有 pptx/模板包场景，用户按需从设置安装）
...
- [docs/decisions/](./docs/decisions/) — Architecture Decision Records (74 ADRs, 001–074)
