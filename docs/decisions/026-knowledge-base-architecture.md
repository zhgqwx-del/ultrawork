...
**UX 优化**（✅ 已实现 2026-05-22，不改变认证机制，降低手动输入摩擦）：

1. **一键打开凭证页面**：凭证输入页面顶部"在浏览器中打开凭证页面"按钮，用 `@tauri-apps/plugin-opener` 的 `openUrl` 打开系统浏览器
2. **凭证复用**：当用户已有一个 IMA 源时，添加新 IMA 源自动预填 clientId（apiKey 因安全脱敏需重新输入），减少一半手动输入
3. **凭证持久化提示**：首次 IMA 源添加成功后 toast 提示"IMA 凭证已保存，下次添加无需重复输入"，仅首次触发
