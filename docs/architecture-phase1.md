...
> | @agent/orchestrator | ✅ 阶段3 全量已实现（2026-06-12） | 编排层（ADR-031）：spawn/await/steer/cancel 原语 + 治理护栏（maxConcurrent/maxDepth=1/超时/子会话 tools deny）+ DAG 调度（Pipeline/Fan-out 同一执行器，按步 worktree 隔离）+ agent 驱动 delegate（`delegate-mcp` stdio shim → 阻塞 `POST /orchestration/delegate` 回 D-2 契约；ACP per-session / opencode 全局双注入）+ QueueOwner 实现；宿主在 ACP sidecar :4099（`/orchestration/*` HTTP+SSE），Desktop 独立路由 `/orchestration` + 主对话 delegate 卡片/dock（不侵入主聊天） |
