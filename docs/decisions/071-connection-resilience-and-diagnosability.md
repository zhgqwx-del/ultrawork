...
- **F1b：断流期间的正文补拉未做（2026-07-30 已量化 → discussions/058）。** **实测比本条原措辞严重得多**：断流落在回合内会自愈（缺失 0），但**跨越回合结束时丢失起点之后的 97.7% 正文且永不自愈**（300 marker 只剩 7）。sidecar 崩溃必然属于后者。修法边界也已澄清：该场景下会话已 idle，而代码注释明说 idle 会话可完全信任服务端快照，**不必碰那条危险的分页合并**。原文： 现在恢复的是状态（转圈、输入框、侧栏），断流窗口里流过的**消息正文**仍会缺一段，直到切走再切回触发全量重取。要动 `use-session-messages` 的分页合并 —— 那里有明确注释警告「重新 seed 会打乱分页顺序并复活已 revert 的轮次」，值得单独一轮设计。
